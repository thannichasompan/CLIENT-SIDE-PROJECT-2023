...
const emits = defineEmits(["currentQueIndex", "task", "score",'incrementScore'])
...
const getAnswer = async () => {
  try {
    const data = await fetch("http://localhost:5000/ansE/")
    if (data.status === 200) {
...
<template>
  ...
  >
    ...
    <div class="w-full h-full flex justify-center items-center flex-col">
      <div class="divcard">
        <div
          class="bg-cover bg-center grid grid-cols-3 grid-rows-3"
