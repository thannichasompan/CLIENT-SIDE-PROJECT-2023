...
const emits = defineEmits(["currentQueIndex", "task", "score","checkSurrender",'incrementScore'])

const router = useRouter()
const setOneUser = useFetchOneUser()
const { mode, buttonarray, currentQueIndex, Ans ,currentQue ,Task , score  } = storeToRefs(setOneUser)
...
const getAnswer = async () => {
  try {
    const data = await fetch("http://localhost:5000/ansH/")
    if (data.status === 200) {
...
<template>
  ...
  >
    ...
    <Narbar :task="Task" :score="score" @checkSurrender="checkSurrender"/>

    
    <div class=" flex justify-center items-center flex-col">
      <div class="divcard">
        <div
          class="bg-cover bg-center grid grid-cols-5 grid-rows-5"
